...
/**
 * SmartUniversityCard — used on the PUBLIC home page.
 * Clicking the card checks if the user has a valid session:
 *  - If authenticated  → navigate to /university/[id]
 *  - If not authenticated → navigate to /auth (sign-in page)
 */
export default function SmartUniversityCard({ uni }: { uni: University }) {
  ...
  const handleClick = () => {
    ...
    if (session) {
      // Authenticated → go to university detail page
      router.push(`/university/${uni.id}`);
    } else {
      // Not authenticated → go to sign-in page with callbackUrl
      router.push(`/auth?callbackUrl=${encodeURIComponent(`/university/${uni.id}`)}`);
    }
  };
